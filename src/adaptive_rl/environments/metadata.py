...
class EnvironmentMetadata(BaseModel):
    ...
    observation_type: str = Field(
        "unknown", description="Type of observation space: 'discrete', 'box', 'dict', etc."
    )
    ...
    max_episode_steps: Optional[int] = Field(
        None, gt=0, description="Default episode truncation step limit"
    )
    reward_range: Tuple[float, float] = Field(
        (-float("inf"), float("inf")), description="Minimum and maximum possible rewards"
    )
    tags: List[str] = Field(
        default_factory=list, description="Categorization tags, e.g. ['discrete', 'navigation']"
    )
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Default parameter dictionary"
    )
